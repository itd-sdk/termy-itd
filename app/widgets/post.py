...
class OriginalPostWidget(PostWidget):
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(classes='post-stats'):
            yield _ClickableStatic(f'{"" if self.post.is_liked else ""} {self.post.likes_count}', classes=f'post-likes{" active" if self.post.is_liked else ""}', id='original-like')
            yield _ClickableStatic(f' {self.post.comments_count}', classes='post-comments', id='original-comment')
            yield _ClickableStatic(f'󰑖 {self.post.reposts_count}', classes=f'post-reposts{" active" if self.post.is_liked else ""}', id='original-repost')
            yield _ClickableStatic(f' {self.post.views_count}', classes=f'post-views{" active" if self.post.is_viewed else ""}', id='original-view')
